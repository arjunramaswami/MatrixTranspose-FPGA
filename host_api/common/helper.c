...
void get_input_data(float2 *transpose_data, float2 *cpu_transpose_data, unsigned N[2], unsigned iterations){
  ...
  for (j = 0; j < iterations; j++){
    for (i = 0; i < (N[0] * N[1]); i++) {
      cpu_transpose_data[(j * N[0] * N[1]) + i].x = transpose_data[(j * N[0] * N[1]) + i].x = (float)i;
      cpu_transpose_data[(j * N[0] * N[1]) + i].y = transpose_data[(j * N[0] * N[1]) + i].y = (float)i;
    }
  }
  #ifdef DEBUG
  for (i = 0; i < iterations * (N[0] * N[1]); i++) {
...
void compute_matrix_transpose(float2 *verify_data, int N[2], unsigned iter){
  unsigned i, j, k;
  float2 *temp = (float2 *)malloc(sizeof(float2) * iter * N[0] * N[1]);

  for(k = 0; k < iter; k++){
    for(i = 0; i < N[0]; i++){
      for(j = 0; j < N[1]; j++){
        temp[(k * N[0] * N[1]) + (j * N[1]) + i].x = verify_data[(k * N[0] * N[1]) + (i * N[1]) + j].x;
        temp[(k * N[0] * N[1]) + (j * N[1]) + i].y = verify_data[(k * N[0] * N[1]) + (i * N[1]) + j].y;
      }
    }
  }

  for(i = 0; i < (iter * N[0] * N[1]); i++){
...
void verify_transpose(float2 *transpose_data, float2 *cpu_transpose_data, int N[2], int iter){
  ...
  for ( i = 0; i < iter * N[0] * N[1]; i++){
    float magnitude = cpu_transpose_data[i].x * cpu_transpose_data[i].x + \
                      cpu_transpose_data[i].y * cpu_transpose_data[i].y;
    float noise = (cpu_transpose_data[i].x - transpose_data[i].x) * (cpu_transpose_data[i].x - transpose_data[i].x) + \
                  (cpu_transpose_data[i].y - transpose_data[i].y) * (cpu_transpose_data[i].y - transpose_data[i].y);
    mag_sum += magnitude;
    noise_sum += noise;

    #ifdef DEBUG
      printf("%d : fpga - (%lf %lf) cpu - (%lf %lf)\n", i, transpose_data[i].x, transpose_data[i].y, cpu_transpose_data[i].x, cpu_transpose_data[i].y);
    #endif
  }
