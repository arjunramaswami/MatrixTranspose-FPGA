...
static int getFolderPath(char **path, int N[2]);
...
cl_program getProgramWithBinary(cl_context context, const cl_device_id *devices, unsigned num_device, int N[2], char *data_path){
  ...
  const char *subpath = "/../fpgabitstream/transpose/synthesis_sp/";
  char *foldername;
  const char *filename = "transpose.aocx";
   
  if( !getFolderPath(&foldername, N) ){
    printf("Path not found for the size (%d,%d)", N[0], N[1]);
    return NULL;
  }
...
int getFolderPath(char **folderPath, int N[2]){
  ...
  if( emu != NULL && emu_int == 1){
    ...
    if( (N[0] == 8) && (N[1] == 8)){
      //char folder[] = "profile/prof3/";
      char folder[] = "synthesis/syn3/";
      *folderPath = malloc(strlen(folder)+1);
      strncpy(*folderPath, folder, strlen(folder));
    }
    else if( N[0] == 16 && N[1] == 16){
      char folder[] = "profile/prof4/";
      //char folder[] = "synthesis/syn4/";
      *folderPath = malloc(strlen(folder)+1);
      strncpy(*folderPath, folder, strlen(folder));
    }
    else if(N[0] == 32 && N[1] == 32){
      char folder[] = "synthesis/syn5/";
      *folderPath = malloc(strlen(folder)+1);
      strncpy(*folderPath, folder, strlen(folder));
    } 
    else if(N[0] == 64 && N[1] == 64){
